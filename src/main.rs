use actix_web::{App, HttpResponse, HttpServer, ResponseError, get, post, http::header, web};
use r2d2::Pool;
...
#[get("/")]
async fn index(db: web::Data<Pool<SqliteConnectionManager>>) -> Result<HttpResponse, TodoAppError> {
  let conn = db.get()?;
